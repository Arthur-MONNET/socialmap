...
app.post('/userTweets', (req, res) => {
    client.v2.userTimeline(req.query['id'], {"expansions": 'geo.place_id',"tweet.fields": ['geo', 'public_metrics', 'created_at'],'place.fields': ['contained_within', 'country', 'geo'], "exclude": ['replies', 'retweets']})
    .then(response =>
        res.send(response))
    .catch(error => console.error(error))
})
